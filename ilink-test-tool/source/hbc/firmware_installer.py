...
import re
import time
import source.hbc.fw_update as fw_update
import source.hbc.get_about as get_about
import source.common_utilities.preload_envfile as preload_envfile
from pathlib import Path
from dotenv import load_dotenv, set_key
from source.hbc.hbc_utils import wait_for_response
...
def check_for_fwinstall_completion():
    ...
    try:
        ...
        output = buf.getvalue()
        buf.close()
        logger.info("Version to be installed is %s", output)
...
def parse_version(output):
    ...
    try:
        ...
        current_version = os.environ["HBC_VERSION"].strip('"')
        if current_version == installed_version:
            ...
            os.environ["HBC_VERSION"] = installed_version
            set_key(dotenv_path, "HBC_VERSION", installed_version)
...
def install_firmware_updates(binary_file):
    ...
    new_version = str(new_version).replace("neo_hbc_fw_bootable_image_OTA_UPGRADE_SIGNED_","")
    ...
    try:
        ...
        handler.flush()
        return f"HBC firmware update complete with v: {new_version}, Previous version was v: {current_version}"
    except Exception as err:
