...
def set_get_fixation_led(intensity_percent, timeout=5):
    ...
    if not hbc:
        msg = "Cannot set Fixation LED controls because HBC is unavailable."
        logger.error(msg)
        return msg

    try:
        # Set Fixation LED controls
        hbc.set_fixation_led_controls(intensity_percent)
        ...
        logger.exception("Failed to set/get Fixation LED controls")
        return "Fixation LED operation failed"