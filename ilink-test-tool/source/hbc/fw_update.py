...
from source.hbc.hbc_utils import logger, get_hbc_instance, reset_hbc_instance

parent_dir = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "neo-hbc-api", "source", "python")
)
...
def update_firmware_from_file(binaryfile):
    ...
    try:
        hbc = get_hbc_instance(rx_msg_handler)
        hbc.update_firmware(binaryfile)
        # After firmware update, reset the HBC instance
        reset_hbc_instance()
    except Exception as e:
