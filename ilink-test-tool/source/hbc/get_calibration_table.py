...
def get_calibration_table_info():
    response_event = threading.Event()
    result = {}

    def handler(msg_type, from_hbc):
        if msg_type == constants.PROTO_CAL_TABLE:
    ...
    try:
        hbc.get_calibration_table()
        wait_for_response()         
        return_data = str(result.get('data'))
        return return_data
        
    except Exception:
        logger.exception("Failed to get calibration table information using get_calibration_table_info")